...
public class qquestions extends JFrame {
	...
	static qquestions frame = new qquestions();
	...
	public qquestions() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try
				{
					FileReader fr=new FileReader("easyRules.txt");//File reader variable set to fr
					BufferedReader br=new BufferedReader(fr);//Buffered reader br reads each line
					String line=br.readLine(); //setting br to line so that it can interact with arrays
					while(line!=null)//searching using while loop, not null condition to check if line is empty
					{
						String[] data=line.split(",");//splitting array by to read through CSV
						qs.add(data[0]);//adding from array to LL
						ans.add(data[1]);
						line=br.readLine();//to avoid constant while loop
					}
					fr.close();
				}
				catch(IOException e1)
